var initialize = function(window, undefined){
  "use strict";
...
var trackingScript = function() { 
  ...
  a.async = 'async'; 
  a.src = '//' + ((window.location.protocol === 'https:') ? 
...
s.innerHTML = '(' + initialize.toString() + ')(window);';
head.appendChild(s);

// add aliasing to document
chrome.runtime.sendMessage({retrieve: 'settings'}, function(resp) {
  var settings = resp.settings || {},
      aliases  = settings.aliases || {},
      tracking = settings.tracking,
      s        = document.createElement('script'),
      inject   = 'window.Sandbox.setAliases(' + JSON.stringify(aliases) + ');';

  s.innerHTML = inject;

  if (tracking)
    s.innerHTML += 'document.readyState === "complete" ? ' + trackingScript.toString() +
      '() : window.addEventListener("load", ' + trackingScript.toString() +
      ', false);';
